...
export function filterByRange(entries: FunnelEntry[], from: string, to: string): FunnelEntry[] {
  return entries.filter((e) => e.datum >= from && e.datum <= to);
}

export function filterByStandort(entries: FunnelEntry[], standort: Standort | 'Alle'): FunnelEntry[] {
...
export function sumByType(entries: FunnelEntry[], typ: Werttyp): number {
  return entries.filter((e) => e.werttyp === typ).reduce((sum, e) => sum + e.anzahl, 0);
}

export function kontakteSum(entries: FunnelEntry[]): number {
  return entries.filter((e) => KONTAKT_TYPEN.includes(e.werttyp)).reduce((sum, e) => sum + e.anzahl, 0);
}
...
export function wiedervorstellungenSum(entries: FunnelEntry[]): number {
  return sumByType(entries, 'wiedervorstellung');
}
